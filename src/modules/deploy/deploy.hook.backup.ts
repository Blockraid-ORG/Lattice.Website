      ...
      try {
        ...
        if (factoryContract?.target) {
          ...
          const amounts = vestings.map((i) =>
            ethers.parseEther(((amountSupply * i.supply) / 100).toString())
          );
          ...
          if (presaleAllocation) {
            ...
            newDate.setDate(
              newDate.getDate() + Number(project.presales.duration)
            );
        ...
        toast.error(`${error?.code ?? "Rejected"}`, {
          description: `${
            error.shortMessage ?? "Action Error, there is something wrong!"
          }`,
          position: "top-center",
        });
      }
