...
import FactoryAbiWithAirdrop from '@/lib/abis/factory-airdrop.abi.json';
...
export function useDeployToken() {
  ...
  const deployFactoryContractWithAirdrop = useCallback(async () => {
    if (typeof window === 'undefined') return
    if (!walletClient || !address) throw new Error('Wallet not connected')
    const provider = new BrowserProvider(walletClient as any)
    const signer = await provider.getSigner(address)
    ...
    const factory = new ethers.ContractFactory(
      FactoryAbiWithAirdrop.abi,
      FactoryAbiWithAirdrop.bytecode,
      signer
    );
  ...
  const deployFactoryWithAirdrop = useCallback(async (project: TProject) => {
    const allocationAirdrop = project.allocations.find(i => i.name === "Airdrop")
    if (!allocationAirdrop) {
      return false;
    }
    const supplyAirdrop = allocationAirdrop.supply
    const airdropClaimStart = dayjs(allocationAirdrop?.startDate).unix()
    const airdropClaimEnd = dayjs(allocationAirdrop?.startDate).add(allocationAirdrop.vesting, "M").unix()
    const totalAirdrop = Number(project.totalSupply) * supplyAirdrop / 100
    const totalAirdropUint = ethers.parseUnits(totalAirdrop.toString(), project.decimals)
    try {
      if (typeof window === 'undefined') return
      if (!walletClient || !address) throw new Error('Wallet not connected')
      const second = 24 * 60 * 60;
      ...
      const factoryContract = await deployFactoryContractWithAirdrop();
      ...
      if (factoryContract?.target) {
        ...
        const factory = new ethers.Contract(factoryContract.target, FactoryAbiWithAirdrop.abi, signer);
        const tokenName = project.name;
        const symbol = project.ticker;
        const initialSupply = project.totalSupply;
  ...
  }, [deployFactoryContractWithAirdrop,
    deployPresale,
    deployProject,
    deployWhitelist,
    setAllocationDeploy,
