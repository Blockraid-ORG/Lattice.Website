...
export default function PresaleItem({ data }: { data: TProject }) {
      ...
      <Link href={`/usr/presale/${data.id}`}>
        ...
        <div className='px-4'>
          ...
          <div className="flex items-center justify-between text-sm mt-3">
            ...
            <div className='flex-1 text-end font-semibold'>{NumberComma(Number(data.totalSupply))}</div>
          </div>
          <div className='mt-3'>
        ...
        <div className='flex justify-end items-center gap-2'>
          {
            data.socials.map((item, index) => (
              <a className='border rounded-lg flex w-7 h-7 items-center justify-center' href={item.url} target="_blank" rel="noopener noreferrer" key={index}>
