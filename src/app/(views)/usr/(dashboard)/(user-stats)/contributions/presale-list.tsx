...
export default function PresaleList() {
  ...
  return (
    <div className="mt-6 bg-chart-gradient p-3 md:p-4 rounded-2xl">
      <div className="mb-2">
        <h2>Your Presale Participation</h2>
      </div>
      {!isLoading ? (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data?.map((item) => (
            <div
              key={item.id}
              className="bg-white border relative p-3 md:p-4 rounded-2xl"
            >
              <GlowingEffect
                borderWidth={1}
                variant="default"
              ...
              <div className="flex gap-2 relative">
                ...
                <div className="flex-1 flex flex-col gap-1">
                  <div className="flex items-center gap-1">
                    <div>{item.ticker}</div>
                    <h3>{item.name}</h3>
                    <div className="bg-black/90  text-white text-[10px] px-2 py-0.5 rounded-full z-20">
