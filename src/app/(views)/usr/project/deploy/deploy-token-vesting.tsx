...
export function DeployTokenVesting({ data }: { data: TProject }) {
  ...
  async function handleDeployContract() {
    ...
    deployToken(data).finally(() => setIsSubmitting(false))
  }
  return (
        ...
        </DialogHeader>
        {
          data && (
            <div>
              <div className="p-4 border rounded-xl mb-6 relative">
                ...
                <div className="flex text-sm">
                  ...
                  <div className="flex-1 font-semibold break-all">
                    <a className="text-sm font-semibold underline text-blue-500 block break-all" href={`${data.chains[0].chain.urlScanner}/address/${data.contractAddress}`} target="_blank" rel="noopener noreferrer">{data.contractAddress}</a>
                  </div>
                </div>
                {
                  isSubmitting && (
              ...
              {/* <div>
                {dataVesting.map(item => (
                  <div key={item.id} className="flex items-center justify-between py-2 border-b border-dashed text-sm">
                    <div className="flex-1">
                      <div>Deploy Allocation {item.name} {`(${item.supply}%)`}</div>
                      <div className="flex items-center gap-2">
                        Contract:
                        <a className="text-xs font-semibold underline text-blue-500 block break-all" href={`${data.chains[0].chain.urlScanner}/address/${item.contractAddress}`} target="_blank" rel="noopener noreferrer">
              ...
              <div>
                {data.allocations.map(item => (
                  <div key={item.id} className="flex items-center justify-between py-2 border-b border-dashed text-sm">
                    <div className="flex-1">
                      <div>Deploy Contract {item.name} {`(${item.supply}%)`}</div>
                      <div className="flex items-center gap-2">
                        Address:
                        {
                          item.contractAddress ? (
