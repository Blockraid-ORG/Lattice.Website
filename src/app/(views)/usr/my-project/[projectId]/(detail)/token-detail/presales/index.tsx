    ...
    <div>
      <div className='my-3 flex justify-between items-center flex-wrap gap-2'>
        ...
        <Button size={'lg'} asChild>
          <Link href={`/usr/my-project/${data.id}/presales`}><Settings /> Manage Presale</Link>
        </Button>
      </div>
      <div className="mt-2">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {
            data.presales.map((item) => (
              <div key={item.id} className='bg-white shadow dark:border dark:bg-slate-900/10 p-4 rounded-xl'>
                  ...
                  <div className='relative py-4 z-10 backdrop-blur-lg h-full bg-white/10 w-full'>
                    {
                      ...
                      ) : dayjs(now).isAfter(item.startDate) && dayjs(now).isBefore(item.endDate) ? (
                        <div className="flex flex-col justify-center gap-1 items-center">
                          End In:
                ...
                <div className='py-2 space-y-1'>
                  ...
                  <PresaleItem
                    ...
                    value={`${item.whitelistDuration ? item.whitelistDuration : '-'} hours`}
                  />
                  <PresaleItem
                    label='Status'
...
const PresaleItem = ({ label, value }: { label: string, value: string | React.ReactNode }) => {
  return (
