...
import {
  TAllocation,
  TProject,
  TProjectAllocationAddress,
} from "@/types/project";
import dayjs from "dayjs";
import { Eye } from "lucide-react";
import { useEffect, useState } from "react";
export function LockerAddressList({
  data,
  project,
}: {
  data: TAllocation;
  ...
  async function handleDeleteByIds() {
    ...
    setDeleting(true);
    resetBeneficiaries(checkedIds, project, unchecked, data)
      .then(() => setOpen(false))
    ...
    <Sheet onOpenChange={onOpenChange} open={open}>
      ...
      <SheetContent side={"bottom"} className="max-h-[90vh]">
        <SheetHeader className="container">
          ...
          <SheetDescription>
            Beneficiary(ies) list who are eligible to claim from the{" "}
            <b>{data.name}</b> locked address. <br />
            Claims start on{" "}
            <b>
              {dayjs(data.startDate).add(2, "day").format("DD-MMM-YYYY")}
            </b>{" "}
            and continue over the next <b>{data.vesting}</b> months.
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="mt-4">
            <div className="mb-4">
              {items.filter((i) => i.isChecked).length > 0 && (
                <Button
                  disabled={data.isFinalized || deleting}
