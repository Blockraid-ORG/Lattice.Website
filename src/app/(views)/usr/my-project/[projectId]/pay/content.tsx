...
export default function PaymentContent() {
  ...
  const { data: project } = useProjectDetail(projectId.toString());
  const { switchChain } = useSwitchChain();
  const [masterPayments, setMasterPayments] = useState<TMasterPayment[] | undefined>()
  useEffect(() => {
            ...
            </div>
            {
              masterPayments ? (
                <div className="mt-4">
                  ...
                  {
                    masterPayments.length > 0 && masterPayments.map(i => (
                      <div key={i.id}>
                        <div className="mb-4">
                          <RowItem label="Amount Due">
                            <div className="flex items-center gap-2">
                              <h2 className="text-lg font-semibold">{i.listingFee}</h2>
                              <div>{i.stableCoin.stableCoin.name} </div>
                            </div>
                          </RowItem>
                          <RowItem label={`Address ${i.stableCoin.stableCoin.name}`}>
                            <div className="break-all">{i.stableCoin.address}</div>
                          </RowItem>
                          <RowItem label="Chain" value={`${i.stableCoin.chain.name}`} />
                        </div>
